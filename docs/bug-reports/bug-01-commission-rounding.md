...
Комиссия = **0 руб.** Кнопка «Перевести» не появляется (потому что условие `(amount + commission) <= available` не выполняется — сравнение сделано строго `>`, см. раздел «Дополнительно»).
...
После ввода суммы 91 — комиссия отображается как 0 руб., кнопка «Перевести» отсутствует:
...
### Сопутствующее наблюдение (не дефект отдельно, FYI)

В том же файле условие появления кнопки «Перевести»:

```js
A = o-s-h-O > 0
```

Сравнение строгое (`>`), хотя по спецификации перевод доступен при `(amount + commission) <= available`, то есть должно быть `>=`. Второстепенное расхождение — отдельно не репорчу.
