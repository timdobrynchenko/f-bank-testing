...
> **О статусе сборки**: красная сборка — это **ожидаемое и требуемое** поведение. Тесты на найденные дефекты намеренно падают — таково условие задания.

## Документация

- [Чек-лист ручного тестирования](docs/checklist.md) — 20 пунктов
- [Тестовые сценарии](docs/test-scenarios.md) — 5 детальных сценариев
- Баг-репорты:
...
|---|---|---|
| Bug #1 | 🟡 Major | Комиссия рассчитывается с округлением до десятков рублей вместо рублей: при сумме 91 руб. комиссия = 0 вместо 9. |
...
**Ожидаемый результат**: `3 failed, 2 passed`. Три теста на дефекты падают по assertion — это требование задания. Два happy/sad path теста проходят и подтверждают, что инфраструктура (драйвер, селекторы, POM) работает корректно.
